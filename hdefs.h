...
inline float __attribute__((always_inline)) node_get_last_activation(Node *n, int inputID) {
    return (n->_train[inputID]._ActiveinputIds != 1) ? 0.0 : n->_train[inputID]._lastActivations;
}

inline void __attribute__((always_inline)) node_set_last_activation(Node *n, int inputID, float realActivation) {
